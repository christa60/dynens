...
parser.add_argument("-k", "--topk", help="top k models to save", default=1)
...
model_name = 'cifar100_%s_model-{epoch:04d}.h5' % model_type
...
for x,v in zip(top_x, top_v):
    ...
    f.write('{},{}\n'.format(name, weight))
    # predicting
    modelname = 'cifar100_{}_model-{:04d}.h5'.format(model_type, x+1)
    ...
    for mc in range(mc_num):    
        ...
        mc_pred.append(pred)
    mc_preds = np.asarray(mc_pred)
    predicts = mc_preds.mean(axis=0)
    # Save predicts
...
for no_top_x_index in no_top_x:
    remove_filename = '{}/cifar100_{}_model-{:04d}.h5'.format(save_dir, model_type, no_top_x_index+1)
