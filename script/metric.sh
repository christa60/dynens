...
# TODO: Confirm whether the below is needed
# Compute consistency
# python -W ignore compute_consistency.py -p1 '../output/cifar10_balance/DS1/simple-ensemble/run_1/prediction_0144.csv' -p2 '../output/cifar10_balance/DS1/simple-ensemble/run_2/prediction_0135.csv' -g '../output/cifar10_balance/DS1/simple-ensemble/run_1/target.csv'
...
datadirs=('cifar100_imbalance') # Note: Change based on experiment (check run.sh script to match)
datafiles=('DS1' 'DS2' 'DS3')
seeds=(22 34 46 58 60) # Note: Change based on experiment and should match the seed setting for the experiment (check run.sh script to match)
runs=('run_1' 'run_2' 'run_3' 'run_4' 'run_5') # Note: Change based on experiment (check run.sh script to match)
...
# ------- ENSEMBLE METHODS --------
# TODO: What does the below do (explanation needed). Do we need Super Ensemble?
# Combine index files
workdir=$WORKDIR/output
datadirs=('cifar100_imbalance') # Note: Change based on experiment (check run.sh script to match)
datafiles=('DS1' 'DS2' 'DS3')
...
# Combination for ensemble methods (to get final output from the ensemble)
workdir=$WORKDIR/output
datadirs=('cifar100_imbalance') # Note: Change based on experiment (check run.sh script to match)
datafiles=('DS1' 'DS2' 'DS3')
...
## Compute consistency for ensemble 
workdir=$WORKDIR/output
datadirs=('cifar100_imbalance') # Note: Change based on experiment (check run.sh script to match)
datafiles=('DS1' 'DS2' 'DS3')
# modeldirs=('simple-ensemble' 'snapshot' 'snapshot-A' 'snapshot-B' 'super-ensemble')
...
workdir=$WORKDIR/output
prefix='/scratch/lw8bn/hybrid_ensemble/output/cifar100_imbalance/DS2/snapshot-A/run_5/'                         # Note: Change based on need
sed -e "s|^|"$prefix"|" < snapshot-A/run_5/index.csv > sensitivity-window-size/n20/index.csv
prefix='/scratch/lw8bn/hybrid_ensemble/output/cifar100_imbalance/DS2/snapshot-A/run_8/'                         # Note: Change based on need
sed -e "s|^|"$prefix"|" < snapshot-A/run_8/index.csv > sensitivity-window-size/n30/index.csv
prefix='/scratch/lw8bn/hybrid_ensemble/output/cifar100_imbalance/DS2/snapshot-A/run_9/'                         # Note: Change based on need
...
prefix='/scratch/lw8bn/hybrid_ensemble/output/cifar100_imbalance/DS2/snapshot-A/run_6/'                         # Note: Change based on need
sed -e "s|^|"$prefix"|" < snapshot-A/run_6/index.csv > sensitivity-snapshot-number/n20/index.csv
prefix='/scratch/lw8bn/hybrid_ensemble/output/cifar100_imbalance/DS2/snapshot-A/run_7/'                         # Note: Change based on need
...
# Combination
workdir=$WORKDIR/output # Note: Change work directory based on need
datadirs=('cifar100_imbalance')
datafiles=('DS1' 'DS2' 'DS3')
for datadir in "${datadirs[@]}"
do
